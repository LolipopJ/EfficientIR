...
class EfficientIR:
    ...
    def img_preprocess(self, image_path):
        ...
        # add batch channel
        norm_img_data = norm_img_data.reshape(
...
class FeatureExtractor:
    """Lightweight model-only wrapper used inside worker processes.

    Loads only the ONNX inference session; never touches the HNSW index.
    ...
    img_preprocess = EfficientIR.img_preprocess
    init_model = EfficientIR.init_model
    get_fv = EfficientIR.get_fv
