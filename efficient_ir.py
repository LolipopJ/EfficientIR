...
class EfficientIR:
    ...
    def img_preprocess(self, image_path):
        ...
        # add batch channel
        norm_img_data = norm_img_data.reshape(1, 3, self.img_size,
                                              self.img_size).astype('float32')
    ...
    def load_index(self):
        if os.path.exists(self.index_path):
            self.hnsw_index.load_index(self.index_path,
                                       max_elements=self.index_capacity)
        else:
            self.hnsw_index.init_index(max_elements=self.index_capacity,
                                       ef_construction=200,
                                       M=48)
